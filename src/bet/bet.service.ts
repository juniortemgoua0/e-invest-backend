...
export class BetService {
  ...
  async findCurrentBet(userId): Promise<Bet | null> {
    const currentBet = await this.betModel.findOne({user: userId}).where({status: BetStatus.IN_PROGRESS})
    if (!currentBet){
      console.log(currentBet)
      throw new HttpException("Vous n'avez pas de mise en cours", HttpStatus.UNAUTHORIZED)
    }
    return currentBet
  }
  ...
  async checkExistingBet(userId: string) {
    const verifyExistingBet = await this.betModel.findOne({user: userId}).where({status: BetStatus.IN_PROGRESS})
    console.log(verifyExistingBet)
    if (verifyExistingBet) {
      throw new HttpException("Il existe deja une mise en cours", HttpStatus.UNAUTHORIZED)
    }
    return {
      msg: 'done',
      status: 200
